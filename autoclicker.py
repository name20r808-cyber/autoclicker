...
def change_window_layer(layer: str) -> None:
    ...
    try:
        ...
        if not hwnd:
            return
        target = {"front": HWND_TOP, "top": HWND_TOPMOST, "back": HWND_BOTTOM}.get(
...
def load_settings() -> Optional[dict]:
    try:
        ...
        if settings.get("countdown_time") is None:
            settings["countdown_time"] = 0
        ...
        if "always_on_top" not in settings:
            ...
            settings_changed = True
        if "always_on_top_toggle_key" not in settings:
            used_keys = {
                key for key in settings.values()
                if isinstance(key, str) and len(key) == 1
            }
            settings["always_on_top_toggle_key"] = next(
                key for key in "axyz" if key not in used_keys
            )
            settings_changed = True
...
def prompt_for_settings() -> dict:
    ...
    always_on_top_toggle_key = read_key(
        "ALWAYS ON TOP TOGGLE: switches the window between top and normal.",
        "a", taken, critical=True,
    )
    taken.append(always_on_top_toggle_key)
    front_key = read_key("BRING TERMINAL TO FRONT.", "f", taken)
    if front_key:
        taken.append(front_key)
    back_key = read_key("SEND TERMINAL TO BACK.", "b", taken)
    if back_key:
    ...
    settings = {
        ...
        "always_on_top_toggle_key": always_on_top_toggle_key,
...
def display_settings(settings: dict) -> None:
    ...
    countdown_display = settings.get("countdown_time") or 0
    print(f" Start waits {countdown_display}s before countdown: YES")
    ...
    print(f" Always on top toggle: {show(settings['always_on_top_toggle_key'])} (cannot be disabled)")
...
def edit_settings(settings: dict) -> dict:
    while True:
        ...
        clear_config_screen()
        print("\n=== CHANGE ONE SETTING ===")
        print(f"1 Countdown seconds [current: {settings.get('countdown_time') or 0}]")
        ...
        print(f"12 Always on top toggle key [current: {show_key(settings['always_on_top_toggle_key'])}]")
        print("13 Restore all default settings")
        print("14 Done and return")
        choice = read_config_input("Choose one setting: ").strip()

        editable_choices = {str(number) for number in range(1, 13)}
        ...
        if choice == "1":
            ...
            key_fields = {
                ...
                "8": ("front_key", "BRING TERMINAL TO FRONT.", False),
                "9": ("back_key", "SEND TERMINAL TO BACK.", False),
            ...
            key_fields = {
                "12": ("always_on_top_toggle_key", "ALWAYS ON TOP TOGGLE: required master control.", True),
            }
            field, description, critical = key_fields[choice]
            taken = [
                value for name, value in settings.items()
            ...
            settings[field] = read_key(description, settings[field], taken, critical=critical)
        elif choice == "13":
            clear_config_screen()
            ...
            print("Choose a number from 1 to 14.")
...
def main() -> None:
    ...
    change_window_layer("top" if settings.get("always_on_top", False) else "normal")
    ...
    print("F brings the terminal front; B sends it back.")
    print(f"{settings['always_on_top_toggle_key'].upper()} overrides F/B and toggles Always on top.")
    for name, key in (
        ...
        ("always on top toggle", settings["always_on_top_toggle_key"]),
    ...
    def on_press(key: object) -> None:
        ...
        if pressed in held_keys:
            return
        held_keys.add(pressed)
        if pressed == settings["global_toggle_key"]:
        ...
        elif pressed == settings["always_on_top_toggle_key"]:
            settings["always_on_top"] = not settings.get("always_on_top", False)
            change_window_layer("top" if settings["always_on_top"] else "normal")
            save_settings(settings)
            state = "ENABLED" if settings["always_on_top"] else "DISABLED"
            ...
            if settings.get("always_on_top", False):
                print("Always on top is active; press the toggle key before using F/B.")
            else:
                change_window_layer("front")
                print("Terminal brought to the front.")
        elif pressed == settings["back_key"]:
            if settings.get("always_on_top", False):
                print("Always on top is active; press the toggle key before using F/B.")
            else:
                change_window_layer("back")
                print("Terminal sent to the back.")
