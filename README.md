...
- `3 Reset all bindings and run first-start setup again`: immediately restore the complete default profile.
...
| `F` | Bring the terminal to the front. Ignored while Always on top is active. |
| `B` | Send the terminal to the back. Ignored while Always on top is active. |
| `A` | Toggle Always on top. This overrides F/B until pressed again. |
...
- Always on top setting: choose option `11`.
- Always on top toggle key: choose option `12`.
- Restore all default settings: choose option `13` and confirm with `y` or `yes`.
- Done and return: choose option `14`.
